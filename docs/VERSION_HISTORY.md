...
|---------|------|---------|
| 5.2.0 | 2026-03-06 | Upload Mode Dialog (processing mode/plugin/baseline selection), conditional tab visibility by processingMode, Step Functions understand-only path runs Compliance Evaluate + Normalizer (was skipping), compliance-evaluate emits processingEvents, PipelineTracker 4th compliance stage, processingMode persisted in DynamoDB, pluginId upload support, baseline name/description inline editing, cleanup.sh --compliance/--plugins/--all flags, JSON parsing fix for LLM text before JSON |
